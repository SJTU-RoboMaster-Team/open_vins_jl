...
bool InertialInitializer::initialize(double &timestamp, Eigen::MatrixXd &covariance, std::vector<std::shared_ptr<ov_type::Type>> &order,
  ...
  if (params.init_max_disparity > 0) {
    ...
    // Refuse to initialize when the visual motion gate cannot be evaluated at
    // all: skipping it lets the estimator come up on a cold feature database
    // with no visual validation, which is how a static ground hold turns into
    // an unobserved divergence.
    int feat_thresh = 15;
